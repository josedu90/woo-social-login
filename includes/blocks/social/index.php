...
function woo_register_social_login_block() {
	...
	if ( function_exists( 'register_block_type' ) ) {
		...
		$args = array(
			...
			'editor_script'   => 'woo-social-login-block-js' ,
			'attributes'      => array(
	            'title'    => array(
	                'type'      => 'text',
	                'default'   => __('Prefer to Login with Social Media', 'wooslg'),
	            ),
	            'networks' => array(
	                'type'      => 'array',
	                'default'   => array(),
	...
	if( $showbuttons ) {
		...
		if( !empty( $attributes['networks'] ) && is_array( $attributes['networks'] ) ){
			...
			foreach ( $attributes['networks'] as $network ) {
				if( !empty( $woo_slg_options['woo_slg_enable_'.$network] ) && $woo_slg_options['woo_slg_enable_'.$network] == 'yes' ){
