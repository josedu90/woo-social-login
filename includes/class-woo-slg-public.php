...
class WOO_Slg_Public
{
        ...
        if (!empty($_POST['email'])) {
            ...
            if (!empty($user)) {
                ...
                if (!empty(array_intersect($user_roles, $admin_roles))) {
                    ...
                    $response['error'] = 1;
                    $response['message'] = __('You can\'t login with admin email for security reasons.', 'wooslg');
                } else {
                    ...
                    if (!empty($woo_slg_enable_email_varification) && $woo_slg_enable_email_varification == 'yes') {
                        ...
                        if (!empty($_woo_slg_user_activate_meta) && $_woo_slg_user_activate_meta == "not_activated") {
                            $response['success'] = 1;
                            $response['message'] = __('A verification link has been sent to your email account. Please verify your email. Did not receive? <a href="javascript:;" onclick="document.getElementById(\'woo-slg-email-login-btn\').click();">Resend it</a>', 'wooslg');
    ...
    public function woo_slg_process_profile($data = array())
    {
        ...
        if (!empty($data) && !empty($data['type'])) {
            ...
            if ($type == 'facebook') {
                ...
                // \WSL\Persistent\WOOSLGPersistent::delete('facebook_at');
                // \WSL\Persistent\WOOSLGPersistent::delete('facebook_display');
    ...
    public function woo_slg_social_login_redirect()
    {
        ...
        if (!is_user_logged_in() && isset($_GET['woo_slg_social_login']) && !empty($socialtype) && array_key_exists($socialtype, $allsocialtypes)) {
            ...
            if ((isset($_POST['woo-slg-submit']) && !empty($_POST['woo-slg-submit']) && $_POST['woo-slg-submit'] == __('Register', 'wooslg'))) {
